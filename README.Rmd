...
Vega-Lite offers a grammar-of-graphics. This package is offers the same declarative approach to building charts as does Vega-Lite; it does not offer functions to build charts using composition. Using this package, you have to provide an entire chart-specification; this means lists of lists.

However, this package may be useful to:

- build re-usable Vega and Vega-Lite specifications for deployment elsewhere.
...
- vegawidget offers Vega-Lite `r vegawidget::vega_version()$vega_lite` and Vega `r vegawidget::vega_version()$vega`. Consequently, this package works only with browsers that support ES6. This includes all modern browsers and the RStudioIDE (version 1.2 and later).

- new function `vw_set_base_url()`, to set the option for [vega-embed](https://github.com/vega/vega-embed)'s default [`baseURL`](https://github.com/vega/vega-loader#loader).

- changes `vega_embed()` to use `...` to pass most of its arguments, making a more-forgiving connection to [vega-embed](https://github.com/vega/vega-embed).
