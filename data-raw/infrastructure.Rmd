...
The current version of {v8} does not have `structuredClone`, which is used in the Vega node-scripts.
Hence, we need to polyfill - for the time being.
...
To remove the polyfill:
  ...
  - remove call to polyfill in `R/to-vega.R`, in `.vw_to_vega.vegaspec_vega_lite()`
