...
The current version of {v8} does not have `structuredClone`, which is
used in the Vega node-scripts. Hence, we need to polyfill - for the time
being.

``` r
dir_polyfill <- path(dir_lib, "polyfill-structuredClone")
dir_create(dir_polyfill)

fs::file_copy(
  fs::path(dir_templates, "polyfill-structuredClone.js"), 
  fs::path(dir_polyfill, "polyfill-structuredClone.js")
)
```

To remove the polyfill:

- remove entries from `data-raw/templates/vegawidget-all.yaml`
- remove `data-raw/templates/polyfill-structuredClone.js`
- remove this section of this RMarkdown file

Rerun this RMarkdown file.
