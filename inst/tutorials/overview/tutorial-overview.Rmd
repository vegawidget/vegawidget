...
If you want to work with the vegawidget package on your own computer, it is not yet on CRAN. You can install it from GutHub, using **devtools**:
...
- set the `x` `encoding` such that the `field` is `"wt"` and the `type` is `"quatitative"`
- set the `y` `encoding` such that the `field` is `"mpg"` and the `type` is `"quatitative"`
...
- set the `value` for `data` to `data_category`
- set the `mark` to `"bar"`
...
One feature shared by Vega-Lite and ggplot2 is the inheritance of plot specifiations. Anything specified at the top level of the spec is inherited in lower levels of the spec.
...
Note that in the JSON, we see the `data` `values` element is column-based (like an R data-frame), rather than row-based (as Vega would expect). This is because, normally, the data frame is not transposed until the vegaspec is rendered. The function that translates from the vegaspec list to JSON is `vw_as_json()`, which wraps `jsonlite::toJSON()` (setting some important paramaters).
...
- for the `x` `encoding`:
  ...
  - set the axis `title` to match the seample chart 
- for the `y` `encoding`:
...
This is just one small part of interactiviy. Reference page, discrete selections. Bound to scales. Bound to external controls.
...
Some warnings about aggregation and a reminder that a tooltip is an encoding.

## What next?

Need to cover concatenated views. Types of selections. Refer to Vega-Lite paper, documentation. Altair package, altair and vegawidget packages. Maps.
