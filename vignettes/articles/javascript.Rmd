...
- [JavaScript](https://javascript.info/) really well written, focused by topic.
- [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript) really well-written, focused by function/object.

## Introduction to vega-view

The [vega-view API](https://github.com/vega/vega/tree/master/packages/vega-view) can modify a Vega chart once it has been rendered. 
  ...
  function(view) {
    ...
    valueGenerator = function*() {
      ...
      while(true) {
        ...
        // extract y values 
        previousY = newVals.map(value => value.y);
        
        // object with elements named x, y, category
        yield newVals;              
      }
