...
If you are working with time-based data, this mismatch of capabilities introduces a lot of "opportunities" to create a Vega(-Lite) visualizations that do not behave as intended. The purposees of this article are 
...
All ISO-8601 strings refer unambigously to UTC, the globally-recognized standard reference-frame; it is the standard way to communicate time-stamps. However, it lacks context for someone who may remember an instant from Chicago, or another who may remember the *same* instant from Paris.
...
For the purpose of making some examples of how to work with UTC dates, let's filter the orginal dataset, with ISO-8601 formatting, keeping the first month:
...
When using UTC dates (and datetimes), be mindful that you may also have to set the scales to UTC, so that the axis and tootips behave similarly, and properly, for everyone.
...
- Weird things may happen at the daylight-saving time transition-points; this problem is compounded if the transistion points are different in the data than they are in the user's browser, e.g. the data's context is in the US and the user's browser is in Europe.

- Your data is now compromised as your datetime strings no longer unabiguously identify the instants in time. Because Vega does not have access to the full list of time-zones, anyone who extracts the data from the Vega specification will be lacking the time-zone context.
...
Let's say you have daily weather data for, say, Seattle over the course of four years. In this case, we want to load the data into vega using a URL, making *this* page a little lighter by not carrying all the data around in each of the follwing charts. Let's have a look at the first few lines:
...
Let's start specification for a time-based scatterplot for the daily maximum-temperature, showing every observation in the dataset. For purposes of illustrating how to work with UTC values, we are going to parse the dates as UTC.
...
- the range of time units to keep, e.g. `"monthdate"`. In this case, time units smaller than *date*, such as hours, minutes and secons are truncated; a "flooring" datetime to the date. In this case, time units larger than *month* are projected into a prototype year, in this case 1900.
