...
If the [**webdriver**](https://github.com/rstudio/webdriver#README) package and PhantomJS are installed, you can use these image-generating functions to create or write images as PNG or SVG, using a vegaspec or vegawidget. This article serves to explain how to use the image-generating functions, as well as confirmation to the developers that they are working.

There seems to be an issue loading data from a URL.



For the image tests, we use explicity test the functions `vw_write_png()` and `vw_write_svg()`. We note the complimentary functions `vw_to_png()` and `vw_to_svg()`; these are implicitly tested as they are the used by the `vw_write_png()` and `vw_write_svg()` functions.
