...
- setting (and soon getting) [data](#data) 
...
These are two separate "things"; the first suggests a function that returns a value, the second suggests a function that produces a side-effect. Accordingly, this package offers a set of JavaScript functions that return values, another set of JavaScript functions that produce side-effects, and a means to compose them.
...
One way we can "hack" our way to a signal is to use the signal into the Vega-Lite specification, then *define* the signal by patching the compiled Vega specification. Vega-Lite developer Dominik Moritz demonstrates this technique in an [Observable notebook](https://beta.observablehq.com/@domoritz/rotating-earth). 
...
In the Vega-Lite spec above, in the `x` `encoding`, we have defined the `bin` `step` as a `signal` named `"bin_width"`. This is not legal in Vega-Lite.
...
Our `output_histogram` will be a vegawidget that is contained in an HTML element with id of `"histogram"`.
...
We also define an output element to tell us the bin-width that Vega is using; we will hook this up using a [signal-listener](https://vega.github.io/vega/docs/api/view/#view_addSignalListener).
...
A signal-handler is a JavaScript function that takes two arguments, the `name` of the signal and the `value` of the signal. Just like with the event-listener in the example above, we define a signal-handler that will return the value of the signal, then put the value into the HTML element with id `"output-bin-width"`, as its text. Note that we are using the `expr` parameter to specify that we want only three decimal places.
...
In the JavaScript chunk below, we define a function that we will want to call whenever the input represented by `input_bin_width` changes.

In this function, `on_bin_width()`, we map the value of the input to the value of the bin-width. Then, using our `Vegawidget.finViewPromise()` function, we set the signal `"bin_width"` in the Vega chart and direct it to re-run.

The `view` is the object of the [vega-view API](https://github.com/vega/vega/tree/master/packages/vega-view#view-api-reference). Here  we [set the signal](https://vega.github.io/vega/docs/api/view/#view_signal) named `"bin_width"` to our new value, then re-[run](https://github.com/vega/vega/tree/master/packages/vega-view#view_run) the view.

Once the function is defined, we direct it to run any time our input-slider changes, and we direct this function to run once at initialization.
...
[Vega](https://vega.github.io/vega/docs/data/) and [Vega-Lite](https://vega.github.io/vega-lite/docs/data.html#named) both offer the ability to name datasets within a specification. We can use the vega-view API to [change a dataset](https://vega.github.io/vega/docs/api/view/#data-and-scales), and in a future version of this package, you will be able to listen to a dataset, i.e. to specify an action whenever a dataset changes.

Let's consider an example with a dataset that has a single observation, and that we can use an HTML input to change that observation. First, we create a range-input that to specify an angle - in our case 0 to 360 degrees. Our input is named `"inp_angle"`.
...
Let's print out our HTML elements to the document, then, like in the signal-example, we will add some JavaScript to define the interactions:
...
Our first job is to create a JavaScript variable that refers to the input slider we created above; we call it `inp_angle` to keep it distinct (in our heads) from the R variable `input_angle`. Please note that the next few code-chunks are `{js}` chunks rather than `{r}` chunks.

```{js}
// this is a {js} chunk

// JS reference to our slider-input
var inp_angle = document.querySelector("[name=inp_angle]");
```
