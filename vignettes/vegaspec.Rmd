...
We also have a function, `vw_examine()`, that you can use to, well, examine a vegaspec. It is a thin wrapper around the `listviewer::jsonedit()` function:
...
When a vegaspec is rendered as a vegawidget, the spec is coerced automatically to JSON by the `vegawidget()` function. Like other JSON-based systems, Vega and Vega-Lite have their own particularities in their JSON format. These are taken into account by the function `vw_as_json()`, which wraps `jsonlite::toJSON()` with the correct defaults for Vega(-Lite). You may find this handy if you want to write a vegaspec to disk or elsewhere.
...
As you can see above, Vega adds on more "room" for axes, labels, legends, etc., so that the entire chart is 357 $\times$ 347 pixels.
...
As you can see above, Vega takes into account the "room" needed for axes, labels, legends, etc., so that the data rectangle is 243 $\times$ 253 pixels.
...
When rendering a Vega-Lite specification, the vega-lite JavaScript library will compile the Vega-Lite specification into Vega. If you have the [**V8**](https://cran.r-project.org/package=V8) package installed, and want to compile a Vega-Lite specification to Vega, for your own purposes, you can use the `vw_to_vega()` function.
