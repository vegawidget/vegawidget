...
```{js}
function getVegaViewTimeout(selector, timeout = 10) {

  var functionGetVegaViewTimeout = function() {

    var view = getVegaView(selector);

    timeout = timeout * 2;

    if (view === null) {
      console.log(
        "Could not find vega view at '" +  selector + "'. " +
        "Waiting " + timeout + " ms to try again."
      );
      view = setTimeout(functionGetVegaViewTimeout, timeout);
    } 
  ...
  return functionGetVegaViewTimeout();
}

var demo_view = getVegaViewTimeout('#streaming-demo');
```
...
window.setInterval(
  function () {
   ...
   var demo_view = getVegaViewTimeout('#streaming-demo', 100);
    ...
    demo_view.change('table', changeSet).run();
  }, 
  1000
);
```
