...
The `vegawidget()` function lets you provide [vega-embed](https://github.com/vega/vega-embed) options. These can be specified as an argument to the `vegawidget()` function, or as an R option: `vega.embed`. The R options `vega.embed`, `vega.height`, and `vega.width` can be useful when printing or creating an image from a spec. 
