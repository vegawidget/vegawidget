...
For the image tests, we use explicity test the functions `vw_write_png()` and `write_svg()`. We note the complimentary functions `to_png()` and `to_svg()`; these are implicitly tested as they are the used by the `vw_write_png()` and `write_svg()` functions.
...
vw_write_png(vw_ex_mtcars, path = file_png)
...
vw_write_png(vw_ex_mtcars, path = file_png, scale = 2)
...
vw_write_png(vw_ex_mtcars, path = file_png, width = 600, height = 300)
...
vw_write_png(vegawidget(vw_ex_mtcars), path = file_png)
